...
class LinearAlgebraVisualization:
    ...
    def visualize_eigenvectors_2d(self, matrix: np.ndarray, eigenvalues, eigenvectors) -> str:
        ...
        try:
            ...
            # Original vectors
            ax.quiver(X, Y, X, Y, alpha=0.3, color='gray', scale=50)
            
            # Transformed vectors
            XY = np.array([X.flatten(), Y.flatten()])
            XY_transformed = matrix @ XY
            X_t = XY_transformed[0].reshape(X.shape)
            Y_t = XY_transformed[1].reshape(Y.shape)
            ax.quiver(X, Y, X_t, Y_t, alpha=0.3, color='blue', scale=50)
            ...
            for i, (val, vec) in enumerate(zip(eigenvalues, eigenvectors.T)):
                ...
                ax.quiver(*origin, *vec_normalized, color=colors[i], 
                         scale=1, scale_units='xy', angles='xy',
                         width=0.015, label=f'λ={val.real:.2f}')
